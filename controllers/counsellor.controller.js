...
export const registerCounsellor = async (req, res) => {
  try {
    const { name, email, password, role, preferredMode, teamOwnerId } =
      req.body;
    if (
      !name ||
      !email ||
...
export const loginCounsellor = async (req, res) => {
  try {
    ...
    const isMatch = await bcrypt.compare(
      password,
      counsellor.counsellor_password,
    );
    if (!isMatch)
...
export const getAllCounsellors = async (req, res) => {
  ...
  try {
    ...
    // If logged-in user is 'to', only show assigned counsellors
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Invalid or missing assignmentType, selectedStudents, or selectedAgents",
      });
    }
    ...
    const updatePromises = selectedStudents.map((studentId, index) => {
      const { counsellorId, name } =
        selectedAgents[index % selectedAgents.length];
    ...
    const logPromises = Object.entries(studentCounsellorMap).map(
...
export async function getCounsellorBreakStats(
  param = {},
  userRole = null,
  userId = null,
) {
  ...
  try {
    if (supervisor_id) {
      ...
      if (!supervisor) {
        return res.status(404).json({
          message:
            'Supervisor not found or not a valid supervisor (role must be "to")',
        });
      }
    }
    ...
    formattedCounsellor.supervisor_name =
      formattedCounsellor.supervisor?.counsellor_name || null;
    ...
    res
      .status(500)
      .json({ message: "Error changing supervisor", error: error.message });
  }
};
