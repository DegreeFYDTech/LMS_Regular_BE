...
export const getStudentsRawSQL = async (filters) => {
  try {
    ...
    // =================== LATEST REMARK FILTERS (APPLIED ON lr.* in main WHERE) ===================
    // Apply remark filters on the latest_remark (lr) in the main query, not in CTE
    ...
    // =================== FIXED CTE: GET ACTUAL LATEST REMARK ===================
    const lRemarkSQL = `
      latest_remark AS (
        ...
        ${selectedagent ? `WHERE sr.counsellor_id = ${escape(selectedagent)}` : ''}
    ...
    if (freshLeads === 'Fresh') {
      ...
      // If not fresh leads but have remark filters, ensure students have latest remarks
