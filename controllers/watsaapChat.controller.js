...
const processTemplatePlaceholders = (template, studentData) => {
  ...
  for (const [key, path] of Object.entries(placeholdersObj)) {
    ...
    const cleanedPath = path.startsWith("student.")
      ? path.replace("student.", "")
      : path;
...
const enhanceMessagePayload = async (messageData, templateName) => {
  ...
  if (
    cachedTemplate &&
    Date.now() - cachedTemplate.timestamp < TEMPLATE_CACHE_TTL
  ) {
...
export const getTemplates = async (req, res) => {
  try {
    ...
    res.status(status).json({
      success: false,
      message: `Failed to fetch templates: ${message}`,
    });
  }
};
...
export const getTemplateById = async (req, res) => {
  try {
    ...
    res.status(status).json({
      success: false,
      message: `Failed to fetch template: ${message}`,
    });
  }
};
...
export const sendWhatsAppTemplate = async (req, res) => {
  ...
  try {
    ...
    const resolvedPlaceholders = processTemplatePlaceholders(
      template,
      studentData,
    );
    ...
    await transaction.rollback();
    const { status, message } = createApiError(
      error,
      "Error sending WhatsApp template",
    );
    res
      .status(status)
      .json({ success: false, message: `Failed to send template: ${message}` });
  }
};
...
export const sendSessionMessage = async (req, res) => {
  ...
  try {
    ...
    if (!sessiondata) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Session data is required" });
    }

    if (!validatePhoneNumber(studentData.student_phone)) {
      await transaction.rollback();
      return res
        .status(400)
        .json({ success: false, message: "Invalid phone number format" });
    ...
    const formattedPayload = {
      ...
      sessiondata: {
        ...sessiondata,
        from: FROM_NUMBER,
        to: ensureCountryCode(studentData.student_phone),
      },
    };
    ...
    if (response.status === 200) {
      try {
        await saveMessageToChat(
          FROM_NUMBER,
          ensureCountryCode(studentData.student_phone),
    ...
    const { status, message } = createApiError(
      error,
      "Error sending session message",
    );
    res
      .status(status)
      .json({ success: false, message: `Failed to send message: ${message}` });
  }
};
...
const updateStudentUnreadCount = async (studentPhone) => {
  try {
    await Student.update(
      {
        number_of_unread_messages: sequelize.literal(
          "COALESCE(number_of_unread_messages, 0) + 1",
        ),
      },
      { where: { student_phone: studentPhone } },
    );
...
export const getMessages = async (req, res) => {
  try {
    ...
    const student = await Student.findOne({
      where: { student_id },
      attributes: ["student_id", "student_phone"],
    });
    const toNumber = student ? ensureCountryCode(student.student_phone) : null;

    if (!toNumber) {
      return res
        .status(400)
        .json({ success: false, message: "toNumber is required" });
    }
  ...
  try {
    ...
    const latestMessageDetails = await Message.findAll({
      where: sequelize.where(
        sequelize.fn(
          "CONCAT",
          sequelize.col("chat_id"),
          "-",
          sequelize.col("timestamp"),
        ),
        {
          [Op.in]: Object.entries(latestTimestamps).map(
            ([chatId, timestamp]) => `${chatId}-${timestamp}`,
          ),
        },
      ),
    ...
    const totalUnreadCount = formattedChats.reduce(
...
export const getMessagesFromPerspective = async (req, res) => {
  try {
    ...
    const chat = await findChatBetween(
    ...
    res.status(500).json({
      success: false,
      message: "Error fetching messages from perspective",
    });
  }
};
  ...
  try {
    ...
    if (otherParticipantNumber) {
      ...
      if (!chat) {
        return res
          .status(200)
          .json({ success: true, count: 0, messages: [], unreadCount: 0 });
      }

      const unreadMessages = await getUnreadMessagesForParticipant(
        chat.chat_id,
        formattedParticipantNumber,
      );
      ...
      const chats = await Chat.findAll({
        where: {
          participants: { [Op.contains]: [formattedParticipantNumber] },
        },
      ...
      const allUnreadMessages = await Message.findAll({
        where: {
          chat_id: chatIds,
          receiver: formattedParticipantNumber,
          is_read: false,
        },
        order: [["timestamp", "DESC"]],
      });
      ...
      chats.forEach((chat) => {
        ...
        const otherParticipant = chat.participants.find(
          (p) => p !== formattedParticipantNumber,
        );
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching unread messages" });
  }
};
