...
export const exportStudentsCSV = async (req, res) => {
  try {
    ...
    const requestedFields = req.query.selectedFields
      ? req.query.selectedFields.split(',').map((f) => f.trim()).filter(Boolean)
      : null;

    const finalFields = requestedFields
      ? validFields.filter((f) => requestedFields.includes(f))
      : validFields;

    const csvData = convertToCSV(validDetails, finalFields, fieldDisplayNames);
