    ...
    if (status === "COMPLETED") {
      ...
      if (payment.payment_for === "application") {
        ...
        const response = await StudentRemark.create({
          ...
          lead_sub_status: "Form Filled_Degreefyd",
          calling_status: "Connected",
          sub_calling_status: "Warm",
          remarks: `Payment of amount ${payment.final_amount} completed successfully.`,
        ...
        const response = await StudentRemark.create({
          ...
          lead_sub_status: "Partially Paid",
          calling_status: "Connected",
          sub_calling_status: "Warm",
          remarks: `Payment of amount ${payment.final_amount} completed successfully.`,
          fees: payment.final_amount,
...
export const getPaymentReports = async (req, res) => {
  try {
    ...
    if (from_date || to_date) {
      ...
      if (to_date)
        paymentWhere.created_at[Op.lte] = new Date(
          new Date(to_date).setHours(23, 59, 59),
        );
    }
    ...
    if (role === "counsellor" || role === "Counsellor") {
      ...
      if (!user_id) {
        return res
          .status(400)
          .json({ message: "User ID is required for Counsellor role" });
      }
